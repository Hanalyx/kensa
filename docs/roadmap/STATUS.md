...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (5 of 15) | ~1.5 weeks | C-024..C-028 shipped (latest `f4f9ddb`). C-028: repeatable `--capability/-C KEY=VALUE` across detect/check/remediate. Overrides the detected capability set before rule selection runs. Vocabulary-validated (unknown KEY error lists all 31 valid keys inline). New `api.ScannerWithOverrides` extension interface; legacy `ScannerBackend` unchanged for backward compat. `internal/detect.{KnownCapabilities,ApplyOverrides}` helpers exposed. Risk-model stanza in spec: this is a debugging knob, not a security boundary; engine atomicity preserves host state regardless of capability lies. Spec at `specs/cli/capability-override.spec.yaml` (6 constraints, 11 ACs). 17 unit tests (7 cmd/kensa + 7 detect + 3 api). Live-verified flipping selinux+apparmor on 192.168.1.211. Next: C-029 (`--workers/-w` parallel SSH). |
