...
| Phase 1 — Multi-call agent binary (`kensa agent --stdio`) | **IN PROGRESS** (L-007..L-013 shipped 2026-05-11; L-014 next — first real handler port) | 4–6 weeks | L-007..L-013 shipped: wirev1 + bridge + echo loop + typed message schema + production framing v2 + AgentClient + protocol version handshake + binary push + SHA cache. **All wire-protocol + transport-lifecycle infrastructure now complete.** L-014 (port file_permissions to agent mode) is the architectural bridge: introduces a local-syscall Transport, RemoteHandler shim, agent-side handler registry, and engine integration. Likely founder-decision territory on integration shape. |
