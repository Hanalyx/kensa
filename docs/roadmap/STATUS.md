...
| Phase 1 — Multi-call agent binary (`kensa agent --stdio`) | **NEARLY COMPLETE** (L-007..L-032 shipped 2026-05-11; L-014c live-host parity + Phase 2 next) | ~1 day actual | L-007..L-032 shipped: full wire stack + LocalTransport + agent-side dispatcher + engine.WithAgentClient + KENSA_USE_AGENT=1 CLI + all 19 capturable handler ports (file_permissions via L-014 + 18 via L-015..L-032 mechanical umbrella close). Audit confirmed ZERO handler code edits needed for Option-L1 drop-in. Remaining: L-014c (live-host parity test gated on `KENSA_TEST_SSH_HOST`). After that, LL Phase 1 is DONE and Phases 2-7 unblocked (file atomicity, deadman rewrite, systemd D-Bus, audit-netlink, sysctl/mount/modprobe direct IO, SELinux). |
