...
| Phase 2 — `--output FORMAT[:PATH]` mechanism (json, jsonl, csv, pdf, evidence, oscal, markdown) | In progress (5 of 10) | ~1 week | C-011..C-013 + C-014 + C-015 shipped (latest `4322089`). C-014: founder ratified `maroto v2` as PDF library on 2026-05-08 (MIT, pure Go, no cgo — preserves static-binary discipline). C-015: pdfScanWriter and pdfRemediationWriter using maroto's row/column DSL with breakline.DashStrategy on the rule cell to handle long no-space SCAP rule IDs. Binary size grew 12MB → 28MB unstripped / 22MB stripped (gofpdf font tables + pdfcpu); follow-up tickets track build-tag gating and `-ldflags="-s -w"` for release builds. PDF intentionally NOT registered for caps/history/JSONValue payloads. Spec context block reframes PDF as operator triage (not audit evidence — that role belongs to C-017 signed envelopes and C-016 OSCAL). Next: C-016 (OSCAL serializer wired through `-o`). |
