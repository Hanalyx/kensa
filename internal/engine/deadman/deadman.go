...
// could disrupt the SSH connection Kensa is using — it arms a deadman timer
// before apply. If the control channel survives apply+validate, the engine
// cancels the timer. If the connection is lost, the scheduled script fires
// autonomously on the target host and restores pre-capture state.
//
// The engine treats a successful arm as PERMISSION TO APPLY. Everything below
// follows from that: a timer this package reports as armed but which cannot
// fire, or which fires a script that restores nothing, is worse than no timer
// at all, because the change is applied either way and the transaction log
// records it as protected.
//
// # Schedulers
//
// Both at(1) and systemd-run are armed where both are usable, because they
// fail differently and neither covers the other.
//
//   - systemd-run is the timer that fires. Its executor is PID 1, so it has no
//     state where the job is accepted and nothing runs it.
//   - at(1) is the backstop across a REBOOT. A systemd-run transient unit lives
//     in tmpfs; at's spool is on disk and atd runs overdue jobs at start.
//
// A scheduler that is installed can still refuse — systemd-run without
// privilege, at(1) with its daemon down — so a refusal advances to the next
// candidate and the arm fails only when every one refuses. See
// [availableSchedulers] and [scheduleSystemdRun] for the measurements behind
// each choice; the non-obvious ones are that systemd's default timer accuracy
// is a MINUTE, and that a relative --on-active window is reset to a full fresh
// window by any daemon-reload — which systemctl enable/disable/mask each
// perform, so the timer was being pushed back by the very apply it guarded.
// The deadline is therefore absolute and computed on the target.
//
// # The script, and where its pieces live
//
// The rollback script is generated by dry-running each capturable step's
// [api.RollbackHandler] against a [recordingTransport], which records shell
// commands without executing them (spec C-02). Three locations, deliberately
// different:
//
//   - The SCRIPT and the fired MARKER live in [scriptDir] under /var/lib, which
//     survives a reboot. Not /tmp: Debian-family hosts clear it at boot before
//     atd starts, which would void the at(1) backstop.
//   - The LOCK lives in [lockDir] under /run, which does NOT survive a reboot.
//     It exists only to stop the two legs running at once within one boot; a
//     lock that outlived a reboot would let a rollback killed mid-flight
//     disable the backstop permanently.
//
// A transaction with NO rollback commands schedules nothing at all — there is
// nothing for a timer to run. That is not treated as a broken capture, because
// several handlers legitimately report success with no work; a capture whose
// values are wrong is caught upstream by servicedbus.CheckPreStateOrientation.
//
// # Cancellation
//
// [Armer.Cancel] asks whether the deadman ALREADY FIRED before it does
// anything else, because both the stop and the script removal destroy the
// evidence, and the answer changes the transaction's verdict: a fired timer
// means the host has already been reverted and must not be recorded as
...
// bare unit name, which resolves to the .service that IS the rollback once the
// timer has elapsed.
//
// # Agent mode
//
// All of the above is the direct-SSH shell path. When an agent client is wired
