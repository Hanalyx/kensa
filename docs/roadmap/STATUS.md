...
| Phase 2 — `--output FORMAT[:PATH]` mechanism (json, jsonl, csv, pdf, evidence, oscal, markdown) | In progress (8 of 10) | ~1 week | C-011..C-018 shipped (latest `b6dc084`). C-018: `--quiet`/`-q` flag suppresses default body output to stdout while preserving stderr (errors, warnings) and exit codes; registered on body-emitting subcommands (detect/check/remediate/rollback/history/plan); intentionally NOT on version/coverage/top-level-version which are operator-explicit info requests. End-to-end test locks the empty-stdout property; cli-smoke.sh grew 41→49 scenarios. Next: C-019 (concurrent fan-out for `-o FORMAT:PATH` end-to-end). |
