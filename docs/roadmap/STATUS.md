...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (6 of 15) | ~1.5 weeks | C-024..C-029 shipped (latest `c6ea837`). C-029: `--workers/-w` int (1-50, default 1) bounds inventory-mode goroutine pool. Replaced unbounded fan-out with generic `fanOutBounded[T]` helper using acquire-before-spawn semaphore. Stderr hint nudges operators with >5 hosts at default 1 toward `-w 5+` (suppressed by --quiet). Out-of-range rejection up front via validateWorkers. Spec at `specs/cli/workers-flag.spec.yaml` (5 constraints, 10 ACs). 11 unit tests (5 fanOutBounded incl. concurrency-peak assertion + 6 validateWorkers). Live-verified `-w 2` against 192.168.1.211. Next: C-030 (`--severity/-s` rule filter). |
