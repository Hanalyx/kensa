...
// RecoverLock is a cross-process advisory lock (flock) that fences
// `kensa recover` against concurrent recovery runs and against a live engine
// on the same store. The lock file sits beside the store db. Recover takes it
// EXCLUSIVE (non-blocking, fails fast if held); a live engine may take it
// SHARED for the duration of its work so an exclusive recover acquisition
// waits for live work to drain rather than racing it.
//
