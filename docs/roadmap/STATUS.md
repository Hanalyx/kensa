...
| Phase 1 — Multi-call agent binary (`kensa agent --stdio`) | **IN PROGRESS** (L-007..L-012 shipped 2026-05-11; L-013 next) | 4–6 weeks | L-007..L-012 shipped: wirev1 + bridge + echo loop + typed message schema + production framing v2 + AgentClient + protocol version handshake. L-013 (binary push + SHA-cached agent caching) is the next pickable deliverable. After L-013, L-014 ports file_permissions as the first real handler — that's the architectural bridge to actual agent-mode remediation. |
