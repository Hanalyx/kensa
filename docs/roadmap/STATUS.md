...
| Phase 3 — Deadman timer rebuild (`timerfd(CLOCK_BOOTTIME)`, `pidfd_open`, `epoll`, `signalfd`) | **IN PROGRESS: 5/7 deliverables done; D-005 design draft awaiting founder review** | ~2 weeks → ~1 day actual through D-004 | **Done:** P-011 (env-var flip, `02bd68d`), D-001 (timerfd, `f804be6`), D-002 (pidfd+probe, `c857b93`), D-003 (signalfd+prctl, `f8facf9`), D-004 (epoll integrator, `0abad1b`), Q4 rename `internal/deadman/` → `internal/engine/deadman/` (`761829b`). **Awaiting two-human review (CONTRIBUTING.md rollback-handler discipline):** D-005 (Armer rewrite — design at `docs/roadmap/D-005-DESIGN.md`). **Blocked on D-005:** D-006 (fuzz harness + Phase 3 close). |
