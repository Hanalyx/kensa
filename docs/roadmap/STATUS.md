...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (12 of 13; 2 deferred) | ~1.5 weeks | C-037 shipped (latest `0234d67`). **C-034 + C-036 deferred to Phase 3.5** (variable-infra design). C-037: long-only repeatable `--rule PATH` on check + remediate. **Loader behavior change**: pre-C-037 --rules-dir XOR explicit files; post-C-037 additive. Strict loading for explicit files; skip-invalid preserved for dir walks. concatPaths helper. Spec: `specs/cli/rule-flag.spec.yaml` (5 constraints, 8 ACs). 8 unit tests. Live-verified single + repeated `--rule` against 192.168.1.211. Remaining: C-038 (Phase 3 close: help-text consolidation + smoke). Next: C-038. |
