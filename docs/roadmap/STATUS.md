...
| Phase 2 — File Atomicity Primitives (`renameat2`, `O_TMPFILE`, `fsync`) | **IN PROGRESS** (P-001 shipped 2026-05-11 `8f73a53`; P-002..P-006 next) | ~2 weeks | P-001 shipped — fsatomic primitive package with concurrent-reader atomicity property verified (14,373 reads × 15 replaces = 0 torn reads). P-002 (file_content), P-003 (file_absent), P-004 (config_set), P-005 (config_set_dropin) can ship in parallel — each is a handler-Apply rewrite requiring per-PR failure-mode analysis per CONTRIBUTING.md. P-006 closes the phase with TRANSACTION_CONTRACT_V1 amendment. |
