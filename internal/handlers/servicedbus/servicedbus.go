...
//   - (nil, ErrHelperNotFound) the helper binary is absent — the caller
//     propagates this so its public method falls back to the shell path.
...
// into the handler's RollbackResult/error pair. err (ErrHelperNotFound or
// a real exec error) propagates unchanged so the caller can fall back;
...
// path's, so Rollback is path-agnostic. ErrHelperNotFound propagates so
