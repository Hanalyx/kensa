...
var deferredCheckCiteMismatches = map[string]string{
	...
	"default-umask|rhel10|V-281082":                "extractor false-positive (verified faithful): rule is config_value on /etc/login.defs key UMASK (emits config:umask target); V-281082 check is `grep -i umask /etc/login.defs` (emits path:/etc/login.defs). Same actual target, different Target.Kind → cannot overlap. Rule requires UMASK==077 == V-281082's requirement.",
