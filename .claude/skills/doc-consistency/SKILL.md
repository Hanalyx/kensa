...
- Call out anything that **changes a shipped verdict** or needs a **consumer
  ...
  `api/` changes explicitly, because they drive the SemVer bump.
...
- Commit messages: imperative present tense; the body explains the mechanism (same
  standard as code comments, meaning no planning labels or chronology); AI-authored commits
  carry a `Co-Authored-By:` trailer.

## SECURITY.md

- It exists, so don't duplicate it. Keep README + CONTRIBUTING linking to it
