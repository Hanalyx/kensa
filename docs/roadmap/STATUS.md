...
| Phase 4 — Session model in store + `kensa diff`, framework `kensa coverage`, `kensa list frameworks`, `kensa info`, `--stats`/`--prune` | **IN PROGRESS** (6 of 12, 2026-05-09) | ~2 weeks | C-039 `21885a7`, C-040 `391d878`, C-041 `aea016f`, C-042 `2b0a61e`, C-043 `3a69700`, C-044 (mechanisms rename) `33db2e4` shipped. Next: C-045 (`kensa coverage` framework control coverage report — repurposes the freed name). Spec corpus 50→57 over the phase so far. |
