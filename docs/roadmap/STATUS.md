...
| Phase 3 — Deadman timer rebuild (`timerfd(CLOCK_BOOTTIME)`, `pidfd_open`, `epoll`, `signalfd`) | **RATIFIED 2026-05-12; P-011 + D-001..D-006 in DELIVERABLES.md** | ~2 weeks | P-011 (Phase 2 env-var flip) lands first. D-001 (timerfd), D-002 (pidfd, probe + PR_SET_PDEATHSIG fallback per Q2.b), D-003 (signalfd + prctl), D-004 (epoll integrator), D-005 (Armer rewrite — agent-mode in-process, direct-SSH retains shell path per Q1.c), D-006 (fuzz harness + Phase 3 close). Founder ratified Q1.c agent-default / Q2.b probe-fallback / Q3.a accept-SIGKILL-risk / Q4 rename `internal/deadman/` → `internal/engine/deadman/`. |
