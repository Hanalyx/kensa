...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (8 of 15) | ~1.5 weeks | C-024..C-031 shipped (latest `3e7e001`). C-031: repeatable `--tag/-t` on check + remediate. Free-form vocabulary (no validation); OR across operator values, AND across filter types (severity narrows first, tags narrow further). Empty-after-tag error discloses pre-tag rule count to disambiguate which filter killed the run. Spec at `specs/cli/tag-filter.spec.yaml` (5 constraints, 11 ACs). 10 unit tests. Live-verified: `-t network` reduced 539-rule corpus to 16 on 192.168.1.211. Next: C-032 (`--category/-c` rule filter). |
