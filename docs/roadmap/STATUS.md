...
| Phase 2 — `--output FORMAT[:PATH]` mechanism (json, jsonl, csv, pdf, evidence, oscal, markdown) | In progress (3 of 10) | ~1 week | C-011 + C-012 + C-013 shipped (latest `84b4d0f`): parser, per-payload writer interfaces, AND CSV serializers (singletons + StreamingCSVScan/Remediation for inventory mode) all in `internal/output/`. Bundled fix in C-013: `kensa history --format csv` trailer routed to stderr so it doesn't corrupt the CSV file. **Loop now paused at C-014 (PDF library decision) — gray-zone, awaiting founder ratification.** |
