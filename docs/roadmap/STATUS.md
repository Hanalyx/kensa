...
| Phase 4 — systemd D-Bus for service handlers | **IN PROGRESS: D-007 done 2026-05-13 (`4e3185e`); 4 deliverables remain (D-008 EnableUnitFiles + service_enabled; D-009 disable; D-010 mask; D-011 close).** | ~3 days remaining | D-008 next (real D-Bus implementation in helper + dual-path handler port). `coreos/go-systemd/v22 v22.7.0` + `godbus/dbus/v5 v5.1.0` portability gates CI-verified on D-007 PR #2. |
