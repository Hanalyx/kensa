...
### 1.9 Privilege model for agent-mode (Phase 4, founder-ratified Option C, 2026-05-13)

- **What.** Per LL Phase 4's privilege-model ratification, the kensa agent process runs as the UNPRIVILEGED SSH user (e.g., `owadmin`), NOT as root. For systemd D-Bus operations the agent invokes `sudo /usr/libexec/kensa-systemd-helper <op> <unit>` as a per-call sudo elevation; the helper opens the system D-Bus as root, performs the operation, prints structured NDJSON, and exits. See `docs/roadmap/PHASE-4-CLOSE.md` for the full rationale (Options A "agent as root" and B "polkit rules" were considered + rejected).
- **What this PROTECTS.** A bug in the agent's wire-protocol parser, framing layer, or non-systemd handler code path cannot be exploited for root code execution — those code paths run unprivileged. The privileged surface is bounded to the `kensa-systemd-helper` binary's argv (enable/disable/mask/start/stop/is-enabled/unit-state) + the file-write paths invoked via `transport.Run` (which sudo-wraps the shell command).
- **What this DOESN'T protect.** The agent binary itself runs at `/var/cache/kensa/agent-<sha>` (root-owned mode 0755 file inside mode 0700 dir, post-B1 fix). If the SSH user's credentials are stolen, an attacker has the same `sudo` powers as the legitimate operator — i.e., everything kensa does. This is the same threat model as `sudo systemctl <whatever>` over SSH today.
- **Bootstrap cache path (B1 fix, 2026-05-13).** Pre-fix, the bootstrap put the binary at `$HOME/.cache/kensa/agent-<sha>` where $HOME was resolved via a sudo'd `printf $HOME` → `/root`, but scp ran as the SSH user → permission denied on `/root/.cache/`. Post-fix, the binary lands in `/var/cache/kensa/` via a stage-then-install dance (scp to `/var/tmp/kensa-stage-<sha>` then sudo `install -m 0755` into the cache). Root-owned cache so SSH-user compromise can't tamper with the cached binary between calls.
- **Sudoers fragment requirement (post-v1.0 packaging).** The kensa-rpm `.spec` file is expected to install `/etc/sudoers.d/kensa-systemd-helper` granting `%kensa ALL=(root) NOPASSWD: /usr/libexec/kensa-systemd-helper`. Operators add their service account to the `kensa` group. AC-09 deferred until the packaging deliverable ships.
- **Sign-off question.** None — Option C ratified 2026-05-13.
