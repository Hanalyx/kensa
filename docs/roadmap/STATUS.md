...
|---|---|---|---|
| Ed25519 signer (task #12, now `M-012`) | **Formal deliverable** (founder-ratified 2026-05-10) | — | See `DELIVERABLES.md` Track M / `M-012`. Loop can pick this up. M-012 ships the cryptographic primitive + `kensa-keygen` binary; C-060 (Phase 5b) then wires it everywhere + adds `kensa verify`. |
...
| Phase 5b — signed-envelope output (Ed25519 through evidence path, `kensa verify` CLI) | **Blocked on M-012** | ~2 days on top of M-012 | C-060 wires the M-012 signer through every engine `EvidenceEnvelope.Signature` write + adds the `kensa verify` subcommand. Ships in v1.1 — v1.0 has no `noopSigner` placeholder claiming to sign. |

### Track M — M7 ship-gate items

| Item | Status | Estimated size | Next action |
|---|---|---|---|
| M-012 — Ed25519 signer (replace noopSigner) | **Pending** (formal deliverable, founder-ratified 2026-05-10) | ~5 days | Ship the cryptographic primitive: `internal/evidence/signer.go` + `cmd/kensa-keygen/`. Tests cover round-trip / tamper-detection / key-rotation. `noopSigner` deleted. `specs/evidence/envelope.spec.yaml` flips draft → active. Loop can pick this up next. |
