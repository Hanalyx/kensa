...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (7 of 15) | ~1.5 weeks | C-024..C-030 shipped (latest `4fba2bc`). C-030: repeatable `--severity/-s` (critical/high/medium/low) on check + remediate. Filters rules at load time before resolveAndPrintIssues sees them. Up-front flag validation; empty filtered set rejects with usage error (documented trade-off; future `--allow-empty` if needed). Case-insensitive both directions. Spec at `specs/cli/severity-filter.spec.yaml` (5 constraints, 11 ACs). 9 unit tests. Live-verified: 539-rule corpus reduced to 70 with `-s critical -s high` on 192.168.1.211. Next: C-031 (`--tag/-t` rule filter). |
