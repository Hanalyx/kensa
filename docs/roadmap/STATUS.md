...
| Phase 3.5 — Rule-variable infrastructure | Not started | 1-2 days | C-034 (--var/-x) + C-036 (--config-dir) deferred here per founder decision 2026-05-09 (commit `72dce47`). Discovery: kensa-go has no rule-variable substitution path; ~30 corpus rules use `{{ var }}` templates and silently fail evaluation today. Phase 3.5 will treat variable infrastructure as a single deliverable that decides defaults location/packaging, which Python tiers to mirror (5-tier priority: CLI > host > group > conf.d > defaults), and the security review for operator overrides flowing into shell commands. |
