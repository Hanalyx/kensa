...
class TestConfigValue:
    def test_key_found_correct_value(self, mock_ssh):
        ...
        check = {"method": "config_value", "path": "/etc/ssh/sshd_config", "key": "PermitRootLogin", "expected": "no"}
    ...
    def test_equals_separator(self, mock_ssh):
        ...
        check = {"method": "config_value", "path": "/etc/security/faillock.conf", "key": "deny", "expected": "3"}
        r = run_check(ssh, check)
        assert r.passed is True
...
class TestFilePermission:
    def test_correct_permissions(self, mock_ssh):
        ssh = mock_ssh({
            "stat": Result(exit_code=0, stdout="root root 0 /etc/shadow", stderr=""),
        })
        check = {"method": "file_permission", "path": "/etc/shadow", "owner": "root", "group": "root", "mode": "0000"}
        r = run_check(ssh, check)
        assert r.passed is True

    def test_wrong_owner(self, mock_ssh):
        ssh = mock_ssh({
            "stat": Result(exit_code=0, stdout="nobody root 600 /etc/shadow", stderr=""),
        })
        check = {"method": "file_permission", "path": "/etc/shadow", "owner": "root", "group": "root", "mode": "0600"}
        r = run_check(ssh, check)
        assert r.passed is False
    ...
    def test_wrong_mode(self, mock_ssh):
        ssh = mock_ssh({
            "stat": Result(exit_code=0, stdout="root root 644 /etc/shadow", stderr=""),
        })
        check = {"method": "file_permission", "path": "/etc/shadow", "owner": "root", "group": "root", "mode": "0000"}
        r = run_check(ssh, check)
        assert r.passed is False
...
class TestCommand:
    def test_exit_code_matches(self, mock_ssh):
        ssh = mock_ssh({
            "authselect current": Result(exit_code=0, stdout="Profile ID: sssd", stderr=""),
        })
    ...
    def test_exit_code_mismatch(self, mock_ssh):
        ssh = mock_ssh({
            "authselect current": Result(exit_code=1, stdout="", stderr="No configuration"),
        })
        check = {"method": "command", "run": "authselect current", "expected_exit": 0}
        r = run_check(ssh, check)
        assert r.passed is False
    ...
    def test_expected_stdout(self, mock_ssh):
        ssh = mock_ssh({
            "cat /etc/hostname": Result(exit_code=0, stdout="webserver-01", stderr=""),
        })
        check = {"method": "command", "run": "cat /etc/hostname", "expected_exit": 0, "expected_stdout": "webserver"}
        r = run_check(ssh, check)
        assert r.passed is True

    def test_expected_stdout_mismatch(self, mock_ssh):
        ssh = mock_ssh({
            "cat /etc/hostname": Result(exit_code=0, stdout="dbserver-01", stderr=""),
        })
        check = {"method": "command", "run": "cat /etc/hostname", "expected_exit": 0, "expected_stdout": "webserver"}
        r = run_check(ssh, check)
        assert r.passed is False
...
class TestSysctlValue:
    def test_matching_value(self, mock_ssh):
        ...
        check = {"method": "sysctl_value", "key": "net.ipv4.ip_forward", "expected": "0"}
        r = run_check(ssh, check)
        assert r.passed is True
...
class TestKernelModuleState:
    def test_blacklisted_and_not_loaded(self, mock_ssh):
        ...
        check = {"method": "kernel_module_state", "name": "cramfs", "state": "blacklisted"}
        r = run_check(ssh, check)
        assert r.passed is True
...
class TestPackageState:
    def test_present_and_installed(self, mock_ssh):
        ssh = mock_ssh({
            "rpm -q": Result(exit_code=0, stdout="aide-0.16-14.el9.x86_64", stderr=""),
        })
    ...
    def test_present_but_not_installed(self, mock_ssh):
        ssh = mock_ssh({
            "rpm -q": Result(exit_code=1, stdout="package aide is not installed", stderr=""),
        })
        check = {"method": "package_state", "name": "aide", "state": "present"}
        r = run_check(ssh, check)
        assert r.passed is False
