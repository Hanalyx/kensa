...
| Phase 2.5 — Operator UX refresh | In progress (2 of 3) | ~2.5 days | C-021 + C-022 shipped (latest `44c8459`). C-022: textScanWriter rewrite per the founder's mockup. FAILED/WARN/PASSED grouped sections, severity badges (CRIT/HIGH/MED/LOW), deepest-common-prefix glob compaction (pam-faillock-* preserves family specificity instead of collapsing to pam-*), fix-line synthesis for 10 common handlers, ASCII progress bar, host banner with rune-counted UTF-8 width. Mechanism-prefix stripping + multi-line script-body detection + "exited with code N (expected M)" extraction = operator-readable details. Live verified on the 539-rule corpus: clean output, "unexpected exit (got 1, want 0)" replacing truncated bash bodies. Next: C-023 polish (OS detection probe, -i, -v/--verbose). |
