...
| Phase 2 — `--output FORMAT[:PATH]` mechanism (json, jsonl, csv, pdf, evidence, oscal, markdown) | In progress (2 of 10) | ~1 week | C-011 + C-012 shipped (latest `639b874`): `internal/output/` now has parser (Parse/ParseAll, 8-format registry) AND writer interfaces (per-payload ScanResultWriter / RemediationResultWriter / HistoryWriter / CapsWriter / JSONValueWriter; text + json + jsonl concrete writers; `*OrText` fallback helpers). cmd/kensa is now flag-parse + transport + dispatch only; print* helpers and scanLine type removed. Behavior preservation verified end-to-end. Next: C-013 (CSV serializer). PDF library decision (C-014) is gray-zone; loop will pause when reached. |
