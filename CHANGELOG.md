...
- **Go 1.26.6 closes five standard-library vulnerabilities Kensa calls.**
  ...
  (`encoding/xml`) and GO-2026-5972 (`encoding/asn1`). The XML finding is
  reached from STIG benchmark parsing and the ASN.1 one from reading an
  evidence verification key, so both sit on paths that read files a host
  operator did not write. The `go` directive in `go.mod` now reads 1.26.6,
  which every CI job and the release build follow, and the scan reports zero
