...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (1 of 15) | ~1.5 weeks | C-024 shipped (`4611058`): short-letter table reconciliation. Breaking changes: `--port` -p→-P, `--sudo` -s→none, `--txn` -t→-T, `--format` -f→none. 9 Phase 3 placeholder constants declared in flags.go; downstream deliverables (C-025..C-038) wire them. CHANGELOG Breaking-changes section live. Help-text examples updated to use long forms or new short letters. Next: C-025 (`--limit/-l` host glob filter). |
