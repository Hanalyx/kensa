...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (10 of 13; 2 deferred) | ~1.5 weeks | C-024..C-033 shipped (latest `e7c2467`). **C-034 + C-036 deferred to Phase 3.5** (founder decision 2026-05-09): kensa-go has no rule-variable substitution path; ~30 corpus rules use `{{ var }}` templates and silently fail today. Variable infrastructure warrants a dedicated design pass (defaults location, Python-tier mirroring, operator-override security review). Phase 3 close (C-038) excludes C-034/C-036. Remaining queue: C-035 (--control), C-037 (--rule), C-038 (Phase 3 close). Next: C-035 (`--control` framework-control filter). |
