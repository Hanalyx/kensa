...
// (the #247 class: every capturable handler that reads whole-file
// content over the shell transport and restores it — file_content, file_absent,
// config_set_dropin, config_append, cron_job, pam_module_configure,
// pam_module_arg, sysctl_set, kernel_module_disable, dconf_set). Encoding the content as base64 round-trips the EXACT bytes:
...
// before any mutation. pathEscaped and sentinel must be pre-escaped by the caller.
func ExistenceReadCmd(testFlag, pathEscaped, sentinel string) string {
	return fmt.Sprintf("if [ %s %s ]; then base64 %s; else printf '%s'; fi",
