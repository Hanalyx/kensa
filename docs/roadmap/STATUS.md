...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (3 of 15) | ~1.5 weeks | C-024 + C-025 + C-026 shipped (latest `f352547`). C-026: `--password/-p` with secure TTY prompt across detect/check/remediate/plan. SSH transport routes through `sshpass -e` (env var only, never argv); reserved sentinel `<prompt>` for prompt mode; non-TTY stdin produces usage error rather than blocking; --password + --inventory rejected (per-host passwords differ); stderr scrubbed for password substrings on connect failure. Spec at `specs/cli/password.spec.yaml` (6 constraints, 9 ACs). 4 unit tests + 54/54 cli-smoke + 37/37 specter check. 2-agent peer review applied. Next: C-027 (`--strict-host-keys` / `--no-strict-host-keys`). |
