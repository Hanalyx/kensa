...
| Phase 4 — Session model in store + `kensa diff`, framework `kensa coverage`, `kensa list frameworks`, `kensa info`, `--stats`/`--prune` | **IN PROGRESS** (5 of 12, 2026-05-09) | ~2 weeks | C-039 (sessions schema) `21885a7`, C-040 (`kensa migrate`) `391d878`, C-041 (`--store` flag) `aea016f`, C-042 (`--stats`) `2b0a61e`, C-043 (`--prune`) `3a69700` shipped. Next: C-044 rename `kensa coverage` → `kensa mechanisms`. Spec corpus 50→56 over the phase so far. |
