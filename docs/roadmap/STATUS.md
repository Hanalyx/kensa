...
| Phase 1 — Multi-call agent binary (`kensa agent --stdio`) | **IN PROGRESS** (L-007..L-014 shipped 2026-05-11; L-014b + L-015 next) | 4–6 weeks | L-007..L-014 shipped: full wire stack + LocalTransport + agent-side dispatcher + engine.WithAgentClient. file_permissions Apply/Capture/Rollback round-trips end-to-end via subprocess agent with observable target-side chmod. **Architectural bridge proof complete.** Remaining: L-014b (CLI env-var wiring) + L-015..L-032 (18 more handler ports). |
