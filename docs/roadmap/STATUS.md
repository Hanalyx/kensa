...
| Phase 2 — `--output FORMAT[:PATH]` mechanism (json, jsonl, csv, pdf, evidence, oscal, markdown) | In progress (6 of 10) | ~1 week | C-011..C-016 shipped (latest `68c55f5`). C-016: OSCAL writer wired through the registry; legacy `--oscal` flag and future `-o oscal:path` (C-018) now share one byte-production path (delegation to `evidence.WriteOSCAL`). Bundled fix: `writeOSCALFile` short-circuits before file creation when no transaction has an envelope so operators don't get a 0-byte OSCAL artifact. Known design deficiency carried forward from M3: per-envelope concatenated documents instead of one combined OSCAL document with multiple results — tracked as a v1.0 ship-blocker follow-up requiring an `evidence.ExportOSCALCombined`. Next: C-017 (Evidence envelope writer). |
