...
| Phase 2 — File Atomicity Primitives (`renameat2`, `O_TMPFILE`, `fsync`) | **NEARLY COMPLETE** (P-001..P-005 shipped 2026-05-11; P-006 docs-close next) | ~1 day actual | P-001 fsatomic primitives + P-002 file_content + P-003 file_absent + P-004 config_set + P-005 config_set_dropin all shipped. All four file-touching capturable handlers now use kernel-atomic operations under agent-mode (direct-SSH falls back to existing shell pipeline). Each migration includes a ratified FMA in its commit body. P-006 (TRANSACTION_CONTRACT_V1 amendment + atomicity-basis tags in CLAUDE.md) closes the phase. |
