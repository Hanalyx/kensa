...
import (
	...
	"github.com/Hanalyx/kensa-go/internal/handlers/sysctlset"
)
...
func TestApply_AC01_RuntimeAndPersistBothWritten(t *testing.T) {
	...
	if !strings.Contains(tp.Runs[1], "/etc/sysctl.d/99-kensa.conf") {
...
func TestCapture_AC03_RecordsRuntimeAndPersistContent(t *testing.T) {
	...
	tp.Results["test -e '/etc/sysctl.d/99-kensa.conf' && cat '/etc/sysctl.d/99-kensa.conf' || printf '__KENSA_ABSENT__'"] =
...
func TestRollback_AC05_RemovesFileWhenAbsent(t *testing.T) {
	...
	if !strings.Contains(tp.Runs[0], "rm -f '/etc/sysctl.d/99-kensa.conf'") {
