...
| Phase 1 — Multi-call agent binary (`kensa agent --stdio`) | **IN PROGRESS** (L-007..L-014b shipped 2026-05-11; L-015 next) | 4–6 weeks | L-007..L-014b shipped: full wire stack + agent-side dispatcher + engine.WithAgentClient + KENSA_USE_AGENT=1 CLI env-var. `kensa remediate` can now route through the agent end-to-end on a live host (live parity test gated on `KENSA_TEST_SSH_HOST`). Remaining: L-014c (live-host parity test wiring) + L-015..L-032 (18 more handler ports). |
