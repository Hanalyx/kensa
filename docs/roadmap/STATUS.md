...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (9 of 15) | ~1.5 weeks | C-024..C-032 shipped (latest `d05fdb8`). C-032: single-value `--category/-c` on check + remediate (NOT repeatable like -s/-t — help text discloses; pflag silently last-write-wins on `-c x -c y`). Case-insensitive exact match. Composes AND with severity + tag (filter chain: severity → tag → category, each empty-after-error includes pre-stage count). Spec at `specs/cli/category-filter.spec.yaml` (5 constraints, 8 ACs). 7 unit tests. Live-verified: `-c logging` reduced 539-rule corpus to 14 on 192.168.1.211. Next: C-033 (`--framework/-f` rule filter — wires into existing `internal/mappings/`). |
