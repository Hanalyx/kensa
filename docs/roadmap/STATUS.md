...
| Phase 2 — `--output FORMAT[:PATH]` mechanism (json, jsonl, csv, pdf, evidence, oscal, markdown) | In progress (7 of 10) | ~1 week | C-011..C-017 shipped (latest `0e92792`). C-017: evidenceRemediationWriter emits each transaction's signed `*api.EvidenceEnvelope` as an indented JSON document (one per non-nil envelope; no wrapper object so each document remains an independently-verifiable signed unit). Pre-M7 the noopSigner placeholder produces zero-length signatures; the wire shape is final and signature production arrives with M7 task #12. PreStateBundle round-trip is locked by an explicit test so a future MarshalJSON change can't silently drop rollback-relevant captured state. Next: C-018 (file-target dispatch wiring `-o FORMAT:PATH` end-to-end). |
