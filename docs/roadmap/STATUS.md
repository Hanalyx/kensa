...
| Phase 4 — systemd D-Bus for service handlers | **IN PROGRESS: D-007 (`4e3185e`) + D-008 (`7d2ca4b`) done 2026-05-13. 4 deliverables remain after scope correction: D-008b (service_enabled handler port + StartUnit + JobRemoved), D-009 (disable), D-010 (mask), D-011 (close).** | ~3 days remaining | D-008b next (handler dual-path; first job-producing op → un-defers AC-05/C-03). `is-enabled` and `unit-state` already use real D-Bus via the helper. |
