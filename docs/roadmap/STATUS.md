...
|---|---|---|---|
| Ed25519 signer (task #12, `M-012` + `C-060`) | **DONE** (M-012 `7036a62` 2026-05-10; C-060 `ca408d0` 2026-05-10) | — | Cryptographic primitive (M-012) + engine wiring + `kensa verify` CLI (C-060) shipped. M7 task #12 fully closed. |
...
| Phase 1 — Multi-call agent binary (`kensa agent --stdio`) | **FUNCTIONALLY COMPLETE** (L-007..L-014c shipped 2026-05-11) | ~1 day actual | All architecture + 19 handler ports + CLI env-var + live-host gating contract shipped. The runs-when-set body of `TestLiveAgentMode_FilePermissionsParity` is a documented TODO awaiting a wired test host; the contract + skip-when-unset path are in place. Phase 2 breakdown drafted in `docs/roadmap/PHASE-2-BREAKDOWN.md` awaiting founder ratification. |
| Phase 2 — File Atomicity Primitives (`renameat2`, `O_TMPFILE`, `fsync`) | **DONE** (corrected re-merge 2026-05-11) | ~2 weeks → 1 day actual | Original P-001..P-005 (`8f73a53`, `51c2bb8`, `fd1fdea`, `888f719`, `ae378a1`) reverted after 4-agent re-review found 9 P0s. Corrected drop landed via `fix/phase-2-rework` branch: fsatomic primitives + relocated `fsatomic.Transport` capability + path-validation + mode-preservation + setuid/setgid/sticky retention + CRLF regex parity + stderr atomicity disclosure + TRANSACTION_CONTRACT §2.6 atomicity-basis matrix. file_permissions chmod stays atomic at syscall level. Follow-ups (P-007 shellEscape consolidation, P-008 §2.6↔code CI gate, founder decision: v1.0 default agent-mode?) tracked in DELIVERABLES.md but not v1.0 blockers. |
| Phase 2 — File atomicity primitives (`renameat2`, `O_TMPFILE`, `fsync`, `syncfs`) | Not started | ~2 weeks | Gates on Phase 1 |
| Phase 3 — Deadman timer rebuild (`timerfd(CLOCK_BOOTTIME)`, `pidfd_open`, `epoll`, `signalfd`) | **CLOSED 2026-05-12: all 7 deliverables shipped (live-host green pending on dedicated test host)** | ~2 weeks → ~1 day actual | **Done:** P-011 (env-var flip, `02bd68d`), D-001 (timerfd, `f804be6`), D-002 (pidfd+probe, `c857b93`), D-003 (signalfd+prctl, `f8facf9`), D-004 (epoll integrator, `0abad1b`), D-005 (Armer rewrite + 2nd-round fix-up, `52a0f21`), D-006 (fuzz harness + Phase 3 close docs). **Phase 3 follow-ups (post-close, both done 2026-05-13):** P-012 specter `coverage --strict` migration (67 specs → Convention B, `f02f243`) + CI gate enable (`d64cf9c`). P-013 close pre-existing CI failures: Lint (357 findings) + CLI Smoke (rule-corpus dependency) via PR #1 (`de81dc7`). |
| Phase 4 — systemd D-Bus for service handlers | **CLOSED 2026-05-13: all 6 deliverables shipped. Option 3 scope: D-Bus primitive layer (helper subcommands: enable/disable/mask/start/stop/is-enabled/unit-state) + `systemd_dbus` capability probe + sudoers convention for kensa-rpm. Handler dual-path consumption deferred to v1.x backlog.** | ~1.5 days actual vs ~1 week sketch | **Done:** D-007 (`4e3185e`), D-008 (`7d2ca4b`), D-009 + D-010 (`f731a20`), D-011 (`8dbb966`), D-012 (this commit). See `docs/roadmap/PHASE-4-CLOSE.md`. |
...
| Phase 5b — signed-envelope output (Ed25519 through evidence path, `kensa verify` CLI) | **DONE** (2026-05-10, `ca408d0`) | ~2 days actual | C-060 shipped: signer wired through engine, noopSigner deleted, KENSA_SIGNING_KEY env var live, `kensa verify <evidence-file>` CLI ships with strict signing_key_id format validation, mode-0600 enforcement on .priv files, symlink rejection in trust dir, kensa-keygen content-addressed alias write. 2-agent peer review caught 5 P0s + 4 P1s, all fixed pre-merge. |

### Track M — M7 ship-gate items

| Item | Status | Estimated size | Next action |
|---|---|---|---|
| M-012 — Ed25519 signer primitive (replaces noopSigner cryptographically) | **DONE** (2026-05-10, `7036a62`) | ~3 days actual | Cryptographic primitive shipped: `internal/evidence/Ed25519Signer` + `cmd/kensa-keygen/` binary. 5 P1s caught + fixed in 2-agent peer review (Severity-canonicalization, alphabetical key sort, reverse-rotation walk, domain separation, --force archive). 19 tests + live end-to-end verification. C-060 unblocked. |
