...
func (h *Handler) Capture(ctx context.Context, transport api.Transport, params api.Params) (*api.PreState, error) {
	...
	cmd := fmt.Sprintf(
		`if [ -e %[1]s ]; then `+
			...
			`base64 %[1]s; `+ // base64 (not cat): the transport trims stdout's trailing
			// newline, which silently dropped a file's final \n and made rollback
			// non-byte-perfect (#247). base64 round-trips the exact bytes.
			`else printf 'ABSENT\n'; fi`,
		shellEscape(p.Path),
	)
	...
	// file's EXACT bytes, trailing newline intact (#247).
	content, decErr := shellcapture.DecodeContent(content)
	if decErr != nil {
		return nil, fmt.Errorf("file_content: capture decode failed for %s: %w", p.Path, decErr)
	}
	return &api.PreState{
		Mechanism:  mechanism,
		Capturable: true,
