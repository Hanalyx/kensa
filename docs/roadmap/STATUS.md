...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (4 of 15) | ~1.5 weeks | C-024..C-027 shipped (latest `8586db3`). C-027: `--strict-host-keys` / `--no-strict-host-keys` flag pair across detect/check/remediate/rollback/plan. SSH transport's `StrictHostKeyChecking` switches yes ↔ accept-new; under strict mode also sets `UpdateHostKeys=no` to block OpenSSH 8.5+ silent key rotation. Mutex check fires before expensive setup (store open, rule parse). Operator hint appended to connect-failure stderr when strict mode rejects an unknown host. Spec at `specs/cli/strict-host-keys.spec.yaml` (5 constraints, 8 ACs). 9 unit tests (5 resolver + 3 masterArgs + 1 subprocess) + 54/54 cli-smoke + 38/38 specter check. Live-verified read-only on 192.168.1.211 in both modes. Next: C-028 (`--capability/-C` capability override). |
