...
| Phase 3 — Full `target_options` + `rule_options` parity | In progress (2 of 15) | ~1.5 weeks | C-024 + C-025 shipped (latest `e8bab57`). C-025: `--limit/-l` host glob filter for inventory mode with ansible-style semantics. parseInventory now tracks [group] memberships; multi-group hosts dedup. Operator-UX guards: include token matching no host → usage error (typo guard); empty result set → usage error. 14 unit tests + 2 parser tests. Next: C-026 (`--password/-p` with secure prompt). |
