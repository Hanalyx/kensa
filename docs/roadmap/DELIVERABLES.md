...
- **Status:** **done** (merged 2026-05-10, feature commit `b97283c`, merge `ca408d0`)
- **Notes:** Two-agent peer review surfaced 5 P0s (commit.go silent Sign() error swallow, path-traversal via envelope.signing_key_id, manpage BUGS section still claiming noopSigner, security.md Verification Protocol step #1 lying after stubs.go cleanup, noopSigner comment tombstone) + 4 P1s (KENSA_SIGNING_KEY mode 0600 enforcement, symlink-rejection in trust dir, missing AC-05/AC-06/AC-07 tests, empty-signing-key-id guard). All addressed pre-merge. The kensa-keygen `--key-id NAME` flag exposed a content-addressed-vs-human-named conflict caught by live test; resolved by writing both `<name>.pub` and `<sha256>.pub` (alias) so `kensa verify`'s spec-C-02 lookup-by-hash works while operators retain human filenames.
...
- **Status:** done (merged 2026-05-10, `7036a62`). Cryptographic primitive shipped: `internal/evidence/Ed25519Signer` + `cmd/kensa-keygen/`. Five P1s caught by 2-agent peer review and fixed before merge: (1) Severity field added to canonical bytes (was excluded — silent forgery vector); (2) canonicalize() now uses map-round-trip for spec-conforming alphabetical key order (was struct-declaration order); (3) rotation history walked reverse-chronological per spec C-04 (was forward); (4) domain-separation tag `kensa-evidence-envelope-v1\0` prefixed to signed bytes (cross-protocol replay defense); (5) `--force` archives prior `.pub` to `<keyid>.pub.archived.<ts>` so auditors verifying past envelopes still have access (was permanent destruction). Plus P2s: KeyIDMismatch warning emission, signature-length pre-check, RFC3339Nano timestamps. 19 tests across signer + keygen. Live-verified end-to-end: kensa-keygen → load PKCS8 PEM via stdlib → Sign → Verify clean → tamper rejected → wrong-key rejected. Deferred to v1.1: revocation list, parent-dir TOCTOU, loose-keydir warning.
...
- **L-014c sub-deliverable: SHIPPED 2026-05-11 (merge `526d585`).** Live-host parity test scaffold + spec + CONTRIBUTING.md env-var documentation. `TestLiveAgentMode_FilePermissionsParity` skips when `KENSA_TEST_SSH_HOST` or `KENSA_TEST_AGENT_MODE=1` is unset; runs the full agent-vs-direct-SSH parity check when both set. Runs-when-set body is a documented TODO awaiting a wired test host; ships as a stub. Spec: `agent-live-host-parity` (C-01..C-04, AC-01..AC-05).
...
- **Size:** ~½ day
- **Status:** **done** (merge `02bd68d`, 2026-05-12)
...
- **Size:** ~1-2 days
- **Status:** **done** (merge `f804be6`, 2026-05-12)
...
- **Size:** ~1 day
- **Status:** **done** (merge `c857b93`, 2026-05-12)
...
- **Size:** ~½ day
- **Status:** **done** (merge `f8facf9`, 2026-05-12 — design pivot to self-pipe + signal.Notify instead of raw signalfd(2); rationale in commit body)
- **Risk:** Low.
...
- **Size:** ~2-3 days
- **Status:** **done** (merge `0abad1b`, 2026-05-12)
...
- **Size:** ~3-4 days
- **Status:** **done** (merge `52a0f21`, 2026-05-12 — original commit `a9d95c3` reviewed; peer review caught 3 P0s + 4 P1s; fix-up commit closes all)
- **Risk:** **High.** Engine-level rollback infrastructure rewrite. Mitigations: keep shell path available; two-human review per CONTRIBUTING; live-host integration test deferred to D-006.
...
- **Size:** ~2-3 days
- **Status:** **done — code-shipped; live-host execution gated on KENSA_FUZZ_HOST + KENSA_FUZZ_DEADMAN_* opt-in flags** (test scaffolding lands in `cmd/kensa-fuzz/deadman_fuzz_test.go`; founder runs against a dedicated test host when one is wired)
- **Risk:** Low-medium. Test infrastructure work. Live-host green is the operational milestone; this deliverable ships the harness + docs in advance.
...
#### D-008 — D-Bus `EnableUnitFiles` + `is-enabled` + `unit-state` helper subcommands + capability probe
- **Phase:** 4
- **Deps:** D-007
- **Acceptance:** Helper's `enable` subcommand calls `dbus.Conn.EnableUnitFilesContext` (synchronous in systemd's D-Bus API), captures the resulting `[]EnableUnitFileChange` symlink list + post-call `UnitFileState`, prints the NDJSON result. Helper's `is-enabled` (AC-07) reads `UnitFileState` and exits 0 on any non-bad/not-found result, 1 otherwise. Helper's `unit-state` (AC-08) returns the rich Capture payload via `GetUnitPropertiesContext` (UnitFileState + ActiveState + SubState + LoadState + FragmentPath + Wants + Requires). D-Bus unreachable surfaces typed `error.code=dbus_unreachable` per AC-11. New capability probe `detect.systemd_dbus` lands in `internal/detect/` (returns true when the system bus socket exists + the kensa-systemd-helper binary is installed + systemctl is available).
- **AC-05 / C-03 stays deferred.** The "subscribe to JobRemoved BEFORE invoking method" guard is for *job-producing* operations (StartUnit, StopUnit, RestartUnit). EnableUnitFiles / DisableUnitFiles / MaskUnitFiles are all synchronous; they return when the filesystem effect is complete, no job created. Re-introduce AC-05 + C-03 when (and if) a deliverable adds StartUnit-style operations (see D-008b below).
- **Scope correction (2026-05-13).** The first-pass D-008 acceptance text claimed the `service_enabled` handler dual-path Apply was in scope. That's incorrect: `service_enabled`'s contract is `systemctl enable --now` — both enable (sync, EnableUnitFiles) AND start (async, requires StartUnit + JobRemoved). The helper has the enable half but not the start half, so D-008's handler port would be incomplete. Split into D-008 (helper subcommands + probe, this entry) and D-008b (handler port, below) so each deliverable ships a coherent unit.
- **Size:** ~1.5 days (helper subcommands + probe + tests).
...
- **Status:** **done** 2026-05-13 — landed via PR #5 (merge commit `8dbb966`). Two commits: `84c88fc` implementation (helper start + stop + shared runJobUnit; 8 new tests covering all 6 systemd completion states + bounded-wait timeout + channel-non-nil contract); `f11df0e` lint follow-up (2 godot capitals on lowercase-identifier-as-sentence-start). All 9 CI jobs green on final run.
- **Risk:** Realized risk was lower than estimated. The fakeConn channel-delivery goroutine pattern modelled the coreos/go-systemd contract cleanly; AC-05's three-part assertion (non-nil + buffered + select-with-timeout) is locked.
...
- **Size:** ~½ day. Doc-only.
- **Status:** **done** 2026-05-13 — this commit. Updates `docs/TRANSACTION_CONTRACT_V1.md §2.6` atomicity matrix to note the Phase 4 D-Bus primitive layer is shipped; refreshes `CLAUDE.md` "Open items before M7 ships" (removes the stale Ed25519/noopSigner items + adds the v1.x handler-port backlog reference); adds `docs/roadmap/PHASE-4-CLOSE.md` capturing the Option C privilege model + sudoers fragment template for the kensa-rpm packaging deliverable; marks all 6 Phase 4 deliverables done.
- **Risk:** None.

**Phase 4 total (Option 3 scope): 6 deliverables, all DONE.** D-007 (`4e3185e`) + D-008 (`7d2ca4b`) + D-009/D-010 (`f731a20`) + D-011 (`8dbb966`) + D-012 (this commit). Total execution: ~1.5 days (vs ~1-week sketch budget). See `docs/roadmap/PHASE-4-CLOSE.md` for the close write-up.
