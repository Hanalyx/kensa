...
- **Rollback output is unsigned.** Same `noopSigner` limit. The "kensa-go enhancement" in the migration doc — emit signed envelopes from `--info` — depends on the M7 signer.
