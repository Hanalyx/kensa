...
|---|---|---|---|
| Ed25519 signer (task #12, `M-012`) | **DONE** (merged 2026-05-10, `7036a62`) | — | Cryptographic primitive shipped. C-060 (Phase 5b) now unblocked: wire signing through engine + ship `kensa verify`. |
...
| Phase 5b — signed-envelope output (Ed25519 through evidence path, `kensa verify` CLI) | **UNBLOCKED** (M-012 shipped 2026-05-10) | ~2 days | C-060 ready to pick up: wire M-012's signer through every engine `EvidenceEnvelope.Signature` write, delete noopSigner from `internal/engine/stubs.go`, migrate engine tests to fixture-real-signer, add the `kensa verify` subcommand. |
