...
| Phase 4 — Session model in store + `kensa diff`, framework `kensa coverage`, `kensa list frameworks`, `kensa info`, `--stats`/`--prune` | **IN PROGRESS** (8 of 12, 2026-05-09) | ~2 weeks | C-039 `21885a7`, C-040 `391d878`, C-041 `aea016f`, C-042 `2b0a61e`, C-043 `3a69700`, C-044 `33db2e4`, C-045 `a194195`, C-046 (`list frameworks`) `5c1f97f` shipped. Next: C-047 `kensa info` (rule/control lookup — largest remaining single subcommand at ~6h). Spec corpus 50→59 over the phase so far. |
