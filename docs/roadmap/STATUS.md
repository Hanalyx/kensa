...
| Phase 2 — `--output FORMAT[:PATH]` mechanism (json, jsonl, csv, pdf, evidence, oscal, markdown) | In progress (9 of 10) | ~1 week | C-011..C-019 shipped (latest `7a84f11`). C-019 wires the headline mechanism end-to-end: `-o`/`--output` (StringSliceP) on detect/check/remediate; `FanOutScanResult`/`FanOutRemediationResult`/`FanOutCaps` dispatch one in-memory result to N destinations concurrently (one goroutine per spec); `ErrUnsupportedFormat` routed to exit 2 via `routeFanOutError`. BLOCKING fixes from peer review: inventory+file-target `-o` rejected with explanatory UsageError to prevent silent per-host data loss; spec example fixed to use scan-appropriate formats. Live verified: `kensa detect ... -o json:/tmp/x.json` produces 687-byte valid JSON, exit 0. cli-smoke.sh grew 49→54. Next: C-020 (deprecation alias for `--format`). |
